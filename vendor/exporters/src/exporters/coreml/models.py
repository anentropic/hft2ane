    ...
    def patch_pytorch_ops(self):
        ...
        def to(context, node):
            ...
            if inputs_len in (4, 5, 7, 8):
                ...
                raise ValueError(
                    "Received invalid arguments for PyTorch conversion of op {}".format(
                        node
                    )
                )
            ...
            if target_dtype is None:
                ...
                types.is_scalar(target_dtype.sym_type) and target_dtype.val is not None
            ):
            ...
            if isinstance(_input, Var) and _input.can_be_folded_to_const():
                ...
                if dtype in NUM_TO_DTYPE_STRING:
                    res = mb.cast(
                        x=_input, dtype=NUM_TO_DTYPE_STRING[dtype], name=node.name
                    )
                else:
                    ...
                    if _np.issubdtype(np_dtype, _np.integer):
                        ...
                        raise ValueError(
                            f"Unsupported op {node} with target dtype {np_dtype}"
                        )
            context.add(res)
