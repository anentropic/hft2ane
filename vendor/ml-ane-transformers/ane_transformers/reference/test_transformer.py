...
TEST_INPUTS = collections.OrderedDict(
    {
        "encoder_input": torch.rand(
            TEST_BATCH_SIZE, TEST_EMBED_DIM, 1, TEST_SRC_SEQ_LEN
        ),
        "decoder_input": torch.rand(
            TEST_BATCH_SIZE, TEST_EMBED_DIM, 1, TEST_TGT_SEQ_LEN
        ),
        "encoder_pos_embed": torch.rand(
            TEST_BATCH_SIZE, TEST_EMBED_DIM, 1, TEST_SRC_SEQ_LEN
        ),
        "decoder_pos_embed": torch.rand(
            TEST_BATCH_SIZE, TEST_EMBED_DIM, 1, TEST_TGT_SEQ_LEN
        ),
        "encoder_k_mask": torch.zeros(TEST_BATCH_SIZE, TEST_SRC_SEQ_LEN, 1, 1),
        "decoder_k_mask": torch.zeros(TEST_BATCH_SIZE, TEST_TGT_SEQ_LEN, 1, 1),
        "encoder_qk_mask": torch.zeros(
            TEST_BATCH_SIZE, TEST_SRC_SEQ_LEN, 1, TEST_SRC_SEQ_LEN
        ),
        "decoder_qk_mask": torch.zeros(
            TEST_BATCH_SIZE, TEST_TGT_SEQ_LEN, 1, TEST_TGT_SEQ_LEN
        ),
    }
)
...
class TestTransformerReferenceImplementation(unittest.TestCase):
    ...
    def test_coreml_conversion_and_speedup(self):
        ...
        try:
            module_traced = torch.jit.trace(self.model, list(self.inputs.values()))
        ...
        try:
            ...
            raise RuntimeError("CoreML conversion targeting ANE failed!") from e
