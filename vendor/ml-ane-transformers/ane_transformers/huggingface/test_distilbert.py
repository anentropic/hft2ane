...
class TestDistilBertForSequenceClassification(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            cls.models = {
                ...
                "ref": transformers.AutoModelForSequenceClassification.from_pretrained(
                    SEQUENCE_CLASSIFICATION_MODEL,
                    return_dict=False,
                    torchscript=True,
                ).eval()
        ...
        cls.models["test"] = ane_transformers.DistilBertForSequenceClassification(
            cls.models["ref"].config
        ).eval()
        ...
        cls.inputs_list = [cls.inputs["input_ids"], cls.inputs["attention_mask"]]
        cls.outputs = {
    ...
    def test_torch_parity(self):
        psnr = testing_utils.compute_psnr(self.outputs["ref"], self.outputs["test"])
        logger.info(
    ...
    def test_torch_accuracy(self):
        ...
        for (text, label), output in zip(
            SEQUENCE_CLASSIFICATION_TEST_SET.items(), self.outputs["test"]
        ):
    ...
    def test_coreml_conversion_and_speedup(self):
        ...
        try:
            module_traced = torch.jit.trace(self.models["test"], self.inputs_list)
        except Exception as e:
        ...
        coreml_out = list(ane_mlpackage_obj.predict(self.np_inputs).values())[0]
        coreml_test_outputs = torch.from_numpy(coreml_out).softmax(1).numpy()

        # Test end-to-end parity for the conversion pipeline
        psnr = testing_utils.compute_psnr(coreml_test_outputs, self.outputs["ref"])
        logger.info(
